...
final class ImageService: Sendable {
    ...
    func landscapeURL(item: BaseItemDto, maxWidth: Int = 600) -> URL? {
        ...
        // 3. Episode Primary (its screenshot) — episodes store their thumbnail as Primary
        if item.type == .episode, tags["Primary"] != nil {
            return posterURL(itemID: id, maxWidth: maxWidth)
        }

        // 4. Parent Thumb (season or series thumb) — most common for episodes
        ...
        // 6. Parent Backdrop (season or series backdrop)
        ...
        // 7. Series Backdrop (via seriesId)
        if item.type == .episode, let seriesID = item.seriesID {
            return backdropURL(itemID: seriesID, maxWidth: maxWidth)
        }

        // 8. Primary image as last resort (portrait, but better than nothing)
        if tags["Primary"] != nil {
            return posterURL(itemID: id, maxWidth: maxWidth)
        }
